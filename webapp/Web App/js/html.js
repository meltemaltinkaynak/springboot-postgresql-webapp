    ...
    try {
        ...
        if (data && data.result && data.data) {
            ...
            async function fetchWebdesignContent() {
                try {
                    const response = await fetch("https://localhost:8443/contents/html", {
                        method: 'GET',
                        credentials: 'include'
                    });
            ...
            async function displayContents(contents) {
                contentContainer.innerHTML = `<h1>Kategori: HTML </h1>`; // Başlık ekle
            ...
            async function fetchWebdesignPublicContent() {
                try {
                    const response = await fetch("https://localhost:8443/contents/public/html", {
                        method: 'GET',
                        credentials: 'include'
                    });
                    if (!response.ok) throw new Error("İçerikler yüklenirken hata oluştu");
