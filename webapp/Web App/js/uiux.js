    ...
    try {
        ...
        if (data && data.result && data.data) {
            ...
            async function fetchWebdesignContent() {
                try {
                    const response = await fetch("https://localhost:8443/contents/uiux", {
                        method: 'GET',
                        credentials: 'include'
                    });
            ...
            async function displayContents(contents) {
                contentContainer.innerHTML = `<h1>Kategori: UI-UX </h1>`; // Başlık ekle
                ...
                for (const content of contents) {
                    ...
                    let imageUrl = "images/default.jpg"; // Varsayılan resim

                    if (content.photo) {
                        if (content.photo.startsWith("/uploads/private/")) {
                            // Private dosyalar için
                            ...
                            // Eğer farklı bir formatta gelirse, olduğu gibi ekleyelim
            ...
            async function fetchWebdesignPublicContent() {
                try {
                    const response = await fetch("https://localhost:8443/contents/public/uiux", {
                        method: 'GET',
                        credentials: 'include'
                    });
                    if (!response.ok) throw new Error("İçerikler yüklenirken hata oluştu");
