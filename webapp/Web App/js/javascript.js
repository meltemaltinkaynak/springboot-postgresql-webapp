    ...
    try {
        ...
        if (data && data.result && data.data) {
            ...
            async function fetchWebdesignContent() {
                try {
                    const response = await fetch("https://localhost:8443/contents/javascript", {
                        method: 'GET',
                        credentials: 'include'
                    });
            ...
            async function displayContents(contents) {
                contentContainer.innerHTML = `<h1>Kategori: JavaScript </h1>`; // Başlık ekle
            ...
            async function fetchWebdesignPublicContent() {
                try {
                    const response = await fetch("https://localhost:8443/contents/public/javascript", {
                        method: 'GET',
                        credentials: 'include'
                    });
                    if (!response.ok) throw new Error("İçerikler yüklenirken hata oluştu");
